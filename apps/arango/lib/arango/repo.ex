defmodule Arango.Repo do
  defmacro __using__(opts) do
    quote do
      ...
      def prepare_changeset_for_update(changeset, opts \\ []) do
        Arango.Ecto.Repo.Schema.prepare_changeset_for_update(@otp_app, changeset, opts)
      end
        ...
        case res do
          ...
          _ ->
            Arango.Ecto.Repo.Schema.load(
              Arango.Ecto.Adapter,
              struct_.__struct__.__struct__,
              Map.to_list(to_struct(struct_, res))
            )
        end
      end
      def one(query_string) do
